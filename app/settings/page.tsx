...
export default function SettingsPage() {
    ...
    useEffect(() => {
        const unsubscribe = auth.onAuthStateChanged((user) => {
            console.log("Auth State:", user ? "Connected as " + user.email : "Not Connected");
            if (user && !isConnected) {
                updateProfile({
                    isConnected: true,
                    userId: user.uid,
                                    ...
                                    onClick={async () => {
                                        ...
                                        try {
                                            ...
                                            if (error?.code === 'auth/popup-closed-by-user') {
                                                ...
                                                toast.error("Connection failed: " + (error?.message || "Check console"));
                                            }
                                        }
                                    }}
                                    className={`w-full p-5 flex items-center justify-between transition-all active:scale-[0.98] text-left group ${isConnected ? 'cursor-default' : 'hover:bg-black/5 dark:hover:bg-white/5'}`}
                                >
                                    ...
                                    {isConnected ? (
                                        ...
                                        <span className="text-[10px] px-2 py-1 rounded-lg bg-primary/10 text-primary font-black uppercase tracking-wider">Start</span>
                                    )}
                                </button>

                                {isConnected && (
                                    <div className="px-5 py-3 bg-rose-500/5 border-t border-black/5 dark:border-white/5 flex items-center justify-between">
                                        <span className="text-[10px] font-bold text-muted-foreground uppercase">Cloud Access Active</span>
                                        <button
                                            onClick={async (e) => {
                                                e.stopPropagation();
                                                try {
                                            ...
                                            className="text-[10px] font-bold text-rose-500 hover:text-rose-600 transition-colors uppercase tracking-widest bg-rose-500/10 px-3 py-1.5 rounded-lg border border-rose-500/10"
                                        >
                                            Disconnect Account
                                        </button>
                                    </div>
